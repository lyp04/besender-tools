...
- **售后服务 → 维修** (`/single/rp`)：右下角新增独立的维修订单统计，支持按公司查询单日或日期区间；「新订单」使用状态 `1` + 创建时间 `time_type=3`，「进行中」使用状态 `2` + 开始时间 `time_type=1`，「已完成」使用状态 `3` + 完成时间 `time_type=2`；已完成订单再按 `is_perfect=1/0` 分别统计良品/不良品数量及其占完成数量的比例
