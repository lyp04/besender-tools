...
- **DOA 管理页** (`/engineerDoa`) **/ 维修 RP 管理页** (`/engineerRepair`)：按日期（单日/区间）统计「完成」状态订单数量，并显示结果数量及占完成数量百分比；DOA 单独统计显示通过/不通过，RP 单独统计显示良品/不良品，勾选同时统计后才用括号补充另一套叫法；口径固定为完成时间，公司等筛选条件沿用页面顶部自带的筛选；DOA 页可勾选「同时统计 RP」、RP 页可勾选「同时统计 DOA」，给出 DOA + RP 合计
