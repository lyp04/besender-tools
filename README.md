...
- **售后服务 → 维修** (`/single/rp`)：右下角新增独立的维修订单统计，支持按公司查询单日或日期区间；数字按事件时间统计：「新订单」按创建时间，「进行中」按开始时间，「已完成」按完成时间，均不受订单之后流转到其它当前状态影响（例如订单后来已出库，仍计入发生过的完成事件）；已完成订单再按 `is_perfect=1/0` 分别统计良品/不良品数量及其占完成数量的比例；多选公司时保留合计，并在下方缩进列出每家公司的新订单、进行中、已完成、良品及不良品数量，单选公司时保持原有显示
