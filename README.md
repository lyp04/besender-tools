...
`bms.besender.com` 的 Tampermonkey 用户脚本：型号良品/不良品聚合统计、头程入库物料反查、维修订单与 DOA 完成统计。
...
- **维修 RP 管理页** (`/engineerRepair`)：右下角「维修订单统计」支持单日/区间，沿用页面顶部所选公司及其它筛选；「新订单」统计区间内按开始时间新建的全部订单，「进行中」统计当前仍进行中且开始时间落入区间的订单，「已完成」按完成时间统计，并给出良品/不良品数量及各自占完成数量的比例；可选同时统计 DOA 完成结果
- **DOA 管理页** (`/engineerDoa`)：保留按日期统计完成订单、通过/不通过数量及比例，并可选同时统计 RP 完成结果
- **时区**：型号列表/详情/头程入库及维修 RP 页的原始时间按中国时间处理，可切换为美东/美中/美西/德国/澳大利亚/中国或系统时区查看，鼠标悬停显示中国时间原文；维修统计使用独立时区设置，默认美西 `America/Los_Angeles`，会把所选完整本地自然日（含夏令时 23/25 小时边界）换算为中国查询窗口；DOA 保留页面本地时间口径
...
- v1.10.0 — 维修统计新增新订单、进行中、已完成及良品/不良品数量与比例；新订单/进行中按开始时间，完成按完成时间；统计时区默认美西并支持切换，修复夏令时日期边界
