...
- **售后服务 → 维修** (`/single/rp`)：右下角新增独立的维修订单统计，支持按公司查询单日或日期区间；「新订单」限定当前新订单状态并按创建时间统计，「进行中」限定当前进行中状态并按开始时间统计；「已完成」按完成时间统计完成事件，不受订单之后出库等状态流转影响；已完成订单再按 `is_perfect=1/0` 分别统计良品/不良品数量及其占完成数量的比例；多选公司时在新订单、进行中、已完成各项下面缩进列出公司，已完成的公司下面再缩进显示良品/不良品及比例，单选公司时保持原有显示
- **DOA 管理页** (`/engineerDoa`)：保留按日期统计完成订单、通过/不通过数量及比例，并可选同时统计 RP 完成结果
- **时区（先读账号时区）**：BMS 按**登录账号资料的 `timezone`**（`users/userInfo`，IANA 名）输出并解释所有业务时间，`start`/`end` 查询参数也按同一时区解释（2026-08-26 用 `America/Los_Angeles` 与 `Asia/Shanghai` 两个账号并排实测，同一行相差 15 h）。脚本启动及每次查询前先通过页面自身的 axios 读取账号时区（30 分钟缓存、并发共享一次请求），型号列表/详情/头程入库及售后维修 (`/single/rp`) 的页面原始时间按账号时区解析，可切换为美东/美中/美西/德国/澳大利亚/中国或系统时区查看，鼠标悬停显示 BMS 原文及其时区；售后维修统计默认美西 `America/Los_Angeles`，支持切换统计时区，并会把所选完整本地自然日（含夏令时 23/25 小时边界）换算为账号时区的查询窗口；读不到账号时区时暂按 `Asia/Shanghai` 处理并在面板提示（5 分钟后重试）；工程师订单 RP / DOA 保留原有页面本地时间口径
